  ...
  const [, drop] = useDrop(
    () => ({
      ...
      async drop(item, monitor) {
        if (item.parent) {
          console.log('ggkfd itme prent',item)
          return;
        }
        ...
        if (id) {
          ...
          const offsetFromLeftOfWindow = canvasBoundingRect.left;
          const currentOffset = monitor.getSourceClientOffset();
          console.log('jlfjlsjfs',currentOffset.x)
          if(currentOffset && currentOffset.x && currentOffset.y){
            left = Math.round(currentOffset && currentOffset.x && currentOffset.x + currentOffset.x * (1 - zoomLevel) - offsetFromLeftOfWindow);
